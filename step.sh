...
EOF
  ...
  darwin*)
    ...
    sudo openvpn --client --dev tun --proto ${proto} --remote ${host} ${port} --resolv-retry infinite --nobind --persist-key --persist-tun --comp-lzo --verb 3 --ca ca.crt --cert client.crt --key client.key --ns-cert-type server --auth-user-pass user-pass --tls-auth tls-auth.key 1
